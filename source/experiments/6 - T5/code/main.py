...
if __name__ == '__main__':
    ...
    identifier = 't5_transformer_mt'

    # --- 3. We open the data and apply tokenization, with data generator ---

    df = pd.read_csv('./preprocessed_data.csv')
    tokenizer = T5Tokenizer.from_pretrained('t5-small') # You can choose the model size (small, base, large, etc.)
    ...
    csv_logger = TimedCSVLogger(f'./results/training_log/training_log_{identifier}.csv', append=True)
    ...
    model_weights_path = f'./results/weights/weights_{identifier}.best'
    ...
    with open(f'./results/predictions/model_predictions_{identifier}.txt', 'w', encoding='utf-8') as file:
