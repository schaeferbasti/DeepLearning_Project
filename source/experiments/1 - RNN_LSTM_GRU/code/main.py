...
def calculate_metrics(predicted_text, ground_truth_text):
    """ Calculates the metrics for a given predicted text and ground truth text. """
    results = []
    # WER
    wer = WordErrorRate()
...
def create_metric_file(method):
    '''Creates a file to store the metric results'''
...
def average_metric_results(metric_results):
    '''Calculates the average of the metric results'''
    avg_results = []
...
def write_metric_results(results, method):
    '''Writes the metric results to a file'''
...
class TimedCSVLogger(CSVLogger):
    """ A CSVLogger that logs the time taken for each epoch. """
...
if __name__ == '__main__':
    ...
    for i in range(len(method_name)):
        ...
        if train_model == True:
            ...
            current_model.load_weights("./results/weights/weights_" + method_name[i] + ".best.h5")
